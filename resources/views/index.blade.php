        ...
        <div class="col-lg-10 col-md-10 col-12">
            <div class="row slider-row">
                ...
                @foreach($trending as $trend)
                    <div class="row slide @if($s == 1) slide-active @endif" id="slide{{ $s }}" data-id="{{ $s }}">
                        <div class="col-12 col-lg-5 slider-img-col">
                            <a href="{{ route('post',['slug' => $trend->slug]) }}">
                                <div class="col-12 slider-thumb" style="background:url('{{ asset($trend->img) }}')"></div>
                            </a>
                        </div>
                        <div class="col-12 col-lg-7 slider-content-div">
                            <div class="col-12">
                                <a class="slider-title" href="{{ route('post',['slug' => $trend->slug]) }}">
                                    <h1>{{ $trend->title }}</h1>
                                </a>
                            </div>
                            <div class="col-12 slider-para">
                                <p>{{ $trend->getContentChunk($trend->content) }}</p>
                            </div>
                            <div class="col-12 text-center slider-btn">
                                <a href="{{ route('post',['slug' => $trend->slug]) }}" class="btn-blue">Read More</a>
                            </div>
                            ...
                            <div class="col-12 slider-credits">
                                <p>Author: <a href="{{ route('author',['name' => $trend->user->username]) }}">{{ $trend->user->username }}</a></p>
                                <p>Posted At: {{ $trend->created_at->diffForHumans() }}</p>
                            </div>
                        </div><!-- Content Div End -->
                    </div><!-- Content Row End -->
                    @php
        ...
        <div class="container-fluid category-section">
            <div class="row">
                ...
                <div class="col category-btn-div">
                    <a href="{{ route('category.posts',['category' => $category->slug]) }}" class="btn-blue">View All</a>
                </div>
            </div>
        </div>
    @endif
